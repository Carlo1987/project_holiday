...
const container_style = 'style="width: 100%; display:flex; flex-direction:column; gap:30px; font-size:18px;" '; 
const title_style = `style="width:80%;  text-align:center; font-size:23px !important; letter-spacing:1px; margin: 0 auto; color:${color_theme}"`;
const message_style = 'width:100%; text-align:center;';
...
let email = {
    ...
    email_server : "carlo_loi87@yahoo.it",
    ...
    sendToken: function(token, language){
    const token_style = 'style="width: 30%; padding:10px; border: 1px solid black; border-radius: 10px; text-align:center; margin: 0 auto;"';
    let lang = this.setLanguage(language);
    let title = lang.send_token.object;

    let email = `
     <div ${container_style}>
          <div ${title_style}> ${lang.send_token.title} </div> 
           <div style="${message_style}"> ${lang.send_token.message} </div> 
           <div ${token_style}> ${token} </div>
     </div> `;
    ...
    resetPassword: function(name, language){
      ...
      let email = `
           <div ${container_style}>
               <div ${title_style}> ${lang.reset_password.title}! </div>
               <div style="${message_style}"> ${lang.good} ${name}, ${lang.reset_password.message}! </div>
           </div>`;
    ...
    reserve: function(reserve, id, language){
        ...
        if(reserve.advance.set == "true"){
            advance = `${lang.reserve.advance} €${(reserve.advance.value_advance).toFixed(2)}, <U> ${lang.reserve.advance_rest} €${(reserve.advance.rest_advance).toFixed(2)} </U> ${lang.reserve.refund_date} ${reserve.refund.date}`;
        }

        const header = " style='width:100%;' ";
        const main_style = " style='width:80%; margin: 0 auto; display:flex; flex-direction:column; gap:15px;' ";
        const title_header = `style='width:100%; text-align:center; font-size:24px; color:${color_theme}; letter-spacing: 1px;' `;
        const container_img = " style='width:80%; height:210px; margin:0 auto;' ";
        const img_style = " style='width:100%; height:100%; border-radius:8px;' ";
        const titleArticle_style = ` style='color:${color_theme}; font-size:20px;' `;
        const article_style = " style='width:80%; border:1px solid black; border-radius:10px; box-shadow:2px 2px 3px rgb(32, 32, 32); margin:0 auto; padding:8px;' ";
        const price_style = " style='font-size:20px; color:green;' ";
        const refund_style = header;
       //  
        email =  `
         <div ${container_style}>

           <div ${title_style}>  ${lang.reserve.title}!  </div>
           ...
           <div ${main_style}>
              ...
              <div ${header}>
                 ...
                 <div ${container_img}>  <img ${img_style} src='http://localhost:3700/home/get_avatar/${avatar_home}' alt='Immagine_${reserve.home_data.name}'>  </div>
              </div>   
           ...
           <div ${refund_style}> 
           ${lang.reserve.refund1} ${reserve.refund.date},${lang.reserve.refund2}.
           ${lang.reserve.click} <a href='http://localhost:4200/refund/${id}'> ${lang.reserve.here} </a> ${lang.reserve.delete_reserve}.
          </div>   
    ...
    completePayment: function(reserve, language){
      ...
      let email =  `
      <div ${container_style}>
        <div ${title_style}> ${lang.completePayment.title} </div>
        <div ${message_style}>
    ...
    refund_cancelled: function(reserve, total_refund, language){
      ...
      let email =  `
       <div ${container_style}>
         <div ${title_style}> ${lang.refund.title}  </div>
         <div ${message_style}>
          <p>${lang.refund.name} ${reserve.user_data.name} ${reserve.user_data.surname} ${lang.refund.delete}. ${message_refund} </p>
          <p> ${lang.refund.thanks} <strong>3338416149</strong> ${lang.refund.email} <a href='mailto:carlo_loi87@yahoo.it'>carlo_loi87@yahoo.it</a> </p>
           <p> ${lang.refund.soon}!! <img ${img_style} src='http://localhost:3700/images/smile_divertente.jpg' alt='smile'>  </p> 
         </div>
       </div>
      `;
