...
///  quantità letti ////
  ...
  carousel(images:Array<string>){
    const carousel_inner = this.El.nativeElement.querySelector('.carousel-inner');
         ...
         if(index == 0){
          html_inner += `
                <div class="carousel-item active">
                    <img src="${this.url}/get_image/${image}" class="d-block w-100" alt="image">
                    <div class="carousel-caption d-block">
                       <p class="btn btn-dark">1/${images.length}</p>
                    </div>
                </div>
          `;
         }else{
          html_inner += `
                <div class="carousel-item">
                    <img src="${this.url}/get_image/${image}" class="d-block w-100" alt="image">
                    <div class="carousel-caption d-block">
                    <p class="btn btn-dark">${index+1}/${images.length}</p>
                 </div>
                </div>
          `;
         }
     ...
     carousel_inner.insertAdjacentHTML('afterbegin',html_inner); 
